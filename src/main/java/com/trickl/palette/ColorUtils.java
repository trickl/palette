...
import com.trickl.validation.constraints.IntRange;
...
public final class ColorUtils {
  ...
  public static Color compositeColors(Color foreground, Color background) {
    ...
    int r = compositeComponent(foreground.getRed(), fgAlpha, background.getRed(), bgAlpha, a);
    int g = compositeComponent(foreground.getGreen(), fgAlpha, background.getGreen(), bgAlpha, a);
    int b = compositeComponent(foreground.getBlue(), fgAlpha, background.getBlue(), bgAlpha, a);
  ...
  public static double calculateContrast(Color foreground, Color background) {
    if (background.getAlpha() != 255) {
      throw new IllegalArgumentException(
          "background can not be translucent: #" + Integer.toHexString(background.getRGB()));
    }
   ...
   * have a contrast value of at least {@code minContrastRatio} when compared to {@code background}.
   *
  ...
  public static int calculateMinimumAlpha(
      Color foreground, Color background, float minContrastRatio) {
    if (background.getAlpha() != 255) {
      throw new IllegalArgumentException(
          "background can not be translucent: #" + Integer.toHexString(background.getRGB()));
    }
   ...
   * @param outHsl 3-element array which holds the resulting HSL components
   */
  public static void colorToHSL(Color color, @NonNull float[] outHsl) {
    RGBToHSL(color.getRed(), color.getGreen(), color.getBlue(), outHsl);
  }
   ...
   */
  public static Color HSLToColor(@NonNull float[] hsl) {
    final float h = hsl[0];
    final float s = hsl[1];
    final float l = hsl[2];
   ...
   */
  public static void colorToLAB(Color color, @NonNull double[] outLab) {
    RGBToLAB(color.getRed(), color.getGreen(), color.getBlue(), outLab);
  }
   ...
   */
  public static void colorToXYZ(Color color, @NonNull double[] outXyz) {
    RGBToXYZ(color.getRed(), color.getGreen(), color.getBlue(), outXyz);
  }
   ...
   *
   * <p>This method expects the XYZ representation to use the D65 illuminant and the CIE 2° Standard
   * Observer (1931).
   *
   * <ul>
   *   <li>outLab[0] is L [0 ...1)
  ...
  public static void XYZToLAB(
      @FloatRange(from = 0f, to = (float) XYZ_WHITE_REFERENCE_X) double x,
      @FloatRange(from = 0f, to = (float) XYZ_WHITE_REFERENCE_Y) double y,
      @FloatRange(from = 0f, to = (float) XYZ_WHITE_REFERENCE_Z) double z,
      @NonNull double[] outLab) {
    if (outLab.length != 3) {
   ...
   */
  public static Color XYZToColor(
      @FloatRange(from = 0f, to = (float) XYZ_WHITE_REFERENCE_X) double x,
      @FloatRange(from = 0f, to = (float) XYZ_WHITE_REFERENCE_Y) double y,
      @FloatRange(from = 0f, to = (float) XYZ_WHITE_REFERENCE_Z) double z) {
   ...
   * @return int containing the RGB representation
   */
  public static Color LABToColor(
      @FloatRange(from = 0f, to = 100) final double l,
      @FloatRange(from = -128, to = 127) final double a,
      @FloatRange(from = -128, to = 127) final double b) {
    final double[] result = getTempDouble3Array();
  ...
  /**
   * Blend between {@code hsl1} and {@code hsl2} using the given ratio. This will interpolate the
   * hue using the shortest angle.
   *
   * <p>A blend ratio of 0.0 will result in {@code hsl1}, 0.5 will give an even blend, 1.0 will
   * result in {@code hsl2}.
   *
   ...
   *
   * <p>A blend ratio of 0.0 will result in {@code lab1}, 0.5 will give an even blend, 1.0 will
   * result in {@code lab2}.
   *
  ...
  public static void blendLAB(
      @NonNull double[] lab1,
      @NonNull double[] lab2,
      @FloatRange(from = 0.0f, to = 1.0f) double ratio,
