 ...
 *
 * <p>The color space is represented as a 3-dimensional cube with each dimension being an RGB
 ...
 *
 * <p>What makes this different to median-cut is that median-cut divided cubes so that all of the
 * cubes have roughly the same population, where this quantizer divides boxes based on their color
 * volume. This means that the color space is divided into distinct colors, rather than
...
final class ColorCutQuantizer {
  ...
  private class Vbox {
     ...
     *
     * <p>This is calculated by finding the longest color dimension, and then sorting the sub-array
     * based on that dimension value in each color. The colors are then iterated over until a color
     * is found with at least the midpoint of the whole box's dimension midpoint.
     *
  ...
  static void modifySignificantOctet(
      final int[] a, final int dimension, final int lower, final int upper) {
    switch (dimension) {
      case COMPONENT_RED:
      ...
      case COMPONENT_GREEN:
        ...
        for (int i = lower; i <= upper; i++) {
          final int color = a[i];
          a[i] =
              quantizedGreen(color) << (QUANTIZE_WORD_WIDTH + QUANTIZE_WORD_WIDTH)
      ...
      case COMPONENT_BLUE:
        ...
        for (int i = lower; i <= upper; i++) {
          final int color = a[i];
          a[i] =
              quantizedBlue(color) << (QUANTIZE_WORD_WIDTH + QUANTIZE_WORD_WIDTH)
  ...
  private boolean shouldIgnoreColor(int color565) {
    final Color color = approximateToRgb888(color565);
    ColorUtils.colorToHSL(color, mTempHsl);
    return shouldIgnoreColor(color, mTempHsl);
  }

  private boolean shouldIgnoreColor(Swatch swatch) {
    return shouldIgnoreColor(swatch.getColor(), swatch.getHsl());
  }

  private boolean shouldIgnoreColor(Color color, float[] hsl) {
    if (mFilters != null && mFilters.length > 0) {
      for (int i = 0, count = mFilters.length; i < count; i++) {
        if (!mFilters[i].isAllowed(color.getRGB(), hsl)) {
          return true;
        }
      }
    }
  ...
  private static int quantizeFromRgb888(int color) {
    int r = modifyWordWidth((color >> 16) & 0xff, 8, QUANTIZE_WORD_WIDTH);
    int g = modifyWordWidth((color >> 8) & 0xff, 8, QUANTIZE_WORD_WIDTH);
    int b = modifyWordWidth(color & 0xff, 8, QUANTIZE_WORD_WIDTH);
    return r << (QUANTIZE_WORD_WIDTH + QUANTIZE_WORD_WIDTH) | g << QUANTIZE_WORD_WIDTH | b;
  }

  /** Quantized RGB888 values to have a word width of {@value #QUANTIZE_WORD_WIDTH}. */
  static Color approximateToRgb888(int r, int g, int b) {
    return new Color(
